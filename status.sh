#!/bin/bash
# ServerStatus 一键管理脚本 v3.0 (Python版)
# 风格: doubi 菜单 + cppla 1.0.0 界面 + 三网中文延迟 + 强制 Caddy
...
install_python() {
    ...
    # 关键: 安装Python依赖
    yellow "正在安装Python依赖库..."
    $PIP_CMD install flask psutil requests 2>/dev/null
    
    # 验证安装
    if $PY_CMD -c "import flask" 2>/dev/null; then
        green "Flask安装成功"
    else
        red "Flask安装失败，尝试使用国内源..."
        $PIP_CMD install -i https://pypi.tuna.tsinghua.edu.cn/simple flask psutil requests
    fi
...
.container{max-width:1400px;margin:0 auto;background:#ecf0f1;border-radius:8px;overflow:hidden}
...
th{background:#bdc3c7;padding:10px 8px;text-align:center}
...
.refresh-time{background:#ecf0f1;text-align:right;padding:5px 10px;font-size:11px}
</style>
</head>
<body>
<div class="container">
...
<table>
<thead><tr><th>节点名称</th><th>状态</th><th>负载</th><th>内存</th><th>硬盘</th><th>网络(上/下)</th><th>电信延迟</th><th>联通延迟</th><th>移动延迟</th></thead>
...
var html='';for(var i=0;i<nodes.length;i++){var n=nodes[i];var statusClass=(n.online===1)?'online':'offline';var load=n.load||[0,0,0];html+='<tr><td>'+n.name+'</td><td class="'+statusClass+'">'+(n.online===1?'在线':'离线')+'</td><td>'+load[0]+'/'+load[1]+'/'+load[2]+'</td><td>'+(n.memory||0)+'%</td><td>'+(n.disk||0)+'%</td><td>↑'+formatSpeed(n.tx_speed)+'/↓'+formatSpeed(n.rx_speed)+'</td><td class="'+getDelayClass(n.telecom)+'">'+(n.telecom==='--'?'--':n.telecom+'ms')+'</td><td class="'+getDelayClass(n.unicom)+'">'+(n.unicom==='--'?'--':n.unicom+'ms')+'</td><td class="'+getDelayClass(n.mobile)+'">'+(n.mobile==='--'?'--':n.mobile+'ms')+'</td></tr>';}
...
if __name__ == '__main__':
    init_db()
    print("ServerStatus 服务端启动，通信端口: %d" % CONFIG['listen_port'])
...
#!/bin/bash
cd ${CLIENT_PATH}
nohup python client.py \$(cat config.txt) > client.log 2>&1 &
echo \$! > client.pid
EOF
...
install_server() {
    ...
    if check_server_status; then
        ...
        red "服务端启动失败，查看日志: cat ${SERVER_PATH}/server.log"
    fi
...
# 启动/停止/重启
start_server() { ${SERVER_PATH}/start.sh; sleep 1; check_server_status && green "服务端已启动" || red "启动失败"; }
